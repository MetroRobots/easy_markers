...
def get_quat(orientation):
    ...
    if orientation is None:
        ...
        q2 = quaternion_from_euler(orientation[0], orientation[1], orientation[2])
        quat.x = q2[0]
        quat.y = q2[1]
        quat.z = q2[2]
        quat.w = q2[3]
...
class MarkerGenerator:
    ...
    def marker(self, position=None, orientation=None, points=None, colors=None, scale=1.0, color=None):
        mark = Marker()
        mark.header.stamp = self.node.get_clock().now().to_msg()
        ...
        mark.lifetime = rclpy.duration.Duration(seconds=self.lifetime).to_msg()
